...
class DirichletMultinomialStrategy(CtaTemplate):
    author = "Xuanhao"
    ...
    def on_long_bar(self, long_bar: BarData):
        ...
        self.prior_bearish = self.post_bearish/self.am_long.size
    ...
    def on_short_bar(self, short_bar: BarData):
        ...
        sumprior =self.prior_neutral+self.prior_bearish+self.prior_bullish

        post_prob_bull = self.post_bullish/sumpost
        post_prob_bear = self.post_bearish/sumpost
        post_prob_neu = self.post_neutral/sumpost

        prior_prob_bull = self.prior_bullish/sumprior
        prior_prob_bear = self.prior_bearish/sumprior
        prior_prob_neu = self.prior_neutral/sumprior

        if post_prob_bull > prior_prob_bull:
            buy_signal = True

        if post_prob_bear > prior_prob_bear:
            sell_signal = True
